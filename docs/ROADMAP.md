...
当前工程基线是 `v0.16`。下一步不要继续扩大真实执行面，先按下面顺序推进：

1. `v0.10 Real Home Pilot`：只选低风险设备，人工授权真实执行，连续观察稳定性和状态一致性。
2. `v0.17 Adapter SDK & Provider Portability`：标准化 provider -> HCM 接入，让更换承载终端时上层系统无需重写。
3. `v1.0`：补齐 E2E、adapter SDK、发布包和开源发布流程。
