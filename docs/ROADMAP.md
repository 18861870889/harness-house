...
- `关闭书房射灯` 在缺少映射时返回 no-action，不跨房间猜测。
...
当前自动化测试已覆盖 HCM planner/executor、HA service simulator、multi-agent runtime、provider onboarding、intent accuracy、digital twin layers、policy gate 和 Adapter Contract。Adapter 专项测试可运行 `npm run test:adapter`。浏览器/Playwright 类 E2E 后续作为 v1.0 前质量补强。
...
当前工程基线是 `v0.18A`。下一步不要继续扩大真实执行面，先按下面顺序推进：

1. `v0.18B Spatial Home Model Editor`：完成户型、房间和逻辑设备/物理控制器的空间编辑。
2. `v0.19 Assisted Mapping`：补齐自动归房建议和 2D/3D 同步。
3. `v0.10 Real Home Pilot`：只选低风险设备，人工授权真实执行，连续观察稳定性和状态一致性。
4. `v1.0`：补齐 E2E、发布包和开源发布流程，并在有条件时增加 Matter/MQTT 实机认证。
