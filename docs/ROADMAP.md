...
接入语音输入和输出，但不让语音绕过 HCM、Intent Accuracy、Safety Gate、Policy Gate 和审计链路。

建议范围：

- 新增 command source：`source=voice`。
- 语音入口复用 `/api/hcm/command`，不新增独立控制通道。
- 先支持 push-to-talk 或浏览器麦克风输入。
- TTS 先用于朗读状态查询和执行结果。
- 评估小爱音箱可开放能力；如果开放不足，采用独立 STT/TTS 模块。
...
- 语音文本进入同一条 HCM command pipeline。
- voice source 下配置类、高风险和隐私能力保持禁止。
- 语音状态查询只读，不产生设备 service call。
- 所有语音指令进入 audit。
...
从“用户发指令”扩展到“事件触发建议”，但第一阶段只做 shadow-mode 自动化建议，不直接写 HA 自动化。

建议范围：

- 订阅或轮询 provider 状态变化。
- 生成事件快照和候选自动化。
- 用 HA Service Simulator 做自动化 preview。
- 用户审核后再进入 overlay 或 automation 配置。
...
把 Home Assistant 从“当前主要 provider”收敛成标准 adapter 示例，让后续 Matter、MQTT、米家原生或其它 provider 可以往 HCM 范式靠。

建议范围：

- Adapter contract test harness。
- Provider snapshot fixture 格式。
- Capability evidence schema。
- Provider diff / onboarding examples。
- Simulator provider adapter template。

验收：

- 新 provider 可以用 mock graph 通过 contract tests。
- Provider 变更能进入 onboarding plan。
- 上层 planner/runtime 不依赖 provider 原始实体结构。
...
如果接入真实设备 adapter 或真实 pilot，需要额外执行人工授权的只读/真实设备验收清单。当前仓库暂未提供 `test:adapter` 和 `test:e2e` npm script，不能把不存在的脚本写进门禁。

当前自动化测试已覆盖 HCM planner/executor、HA service simulator、multi-agent runtime、provider onboarding、intent accuracy、digital twin layers 和 policy gate。浏览器/Playwright 类 E2E 后续作为 v1.0 前质量补强。
...
当前工程基线是 `v0.14`。下一步不要继续扩大真实执行面，先按下面顺序推进：

1. `v0.10 Real Home Pilot`：只选低风险设备，人工授权真实执行，连续观察稳定性和状态一致性。
2. `v0.15 Voice Interaction Alpha`：把语音作为新的 command source，复用现有安全链路。
3. `v0.16 Event Runtime & Automation Proposals`：事件驱动只生成 shadow proposal，不直接写真实自动化。
4. `v0.17 Adapter SDK & Provider Portability`：沉淀 adapter contract tests，让 HA 可替换。
5. `v1.0`：补齐文档、E2E、adapter SDK 和开源发布流程。
