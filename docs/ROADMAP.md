...
先实现独立 TTS 输出，让 Harness House 能朗读状态查询、执行结果、拒绝原因和确认请求。本版本不做语音输入，不接小爱。
...
- TTS 失败不能影响文字结果和设备执行链路。

非目标：

- 小爱音箱接入。
- STT、麦克风输入和 push-to-talk。
- 生产级常开监听。
- 语音触发设备执行。
...
- 新消息可以按策略打断旧消息。
- TTS 失败时文字 UI、audit 和设备执行结果不受影响。
- TTS 输出不能被系统当成新指令回流。
...
2. `v0.15 TTS Output Alpha`：只做独立语音输出，不接小爱，不做语音输入。
3. `v0.16 Home Event & Automation Suggestions`：观察家庭事件并生成可解释、可模拟的自动化建议，不直接执行。
4. `v0.17 Adapter SDK & Provider Portability`：标准化 provider -> HCM 接入，让更换承载终端时上层系统无需重写。
