...
- 当前 2D 空间编辑结果还没有反向驱动 3D scene 坐标；该同步进入 v0.19。
...
当前工程基线是 `v0.18B`。下一步不要继续扩大真实执行面，先按下面顺序推进：

1. `v0.19 Assisted Mapping`：补齐自动归房建议和 2D/3D 同步。
2. `v0.10 Real Home Pilot`：只选低风险设备，人工授权真实执行，连续观察稳定性和状态一致性。
